...
/**
 * Extract and verify fediverse creator handle from a Fediverse URL using WebFinger.
 * Returns @user@domain format or null if the account cannot be resolved.
 */
export async function extractFediverseCreatorFromURL(
  urlString: string
): Promise<string | null> {
  try {
    ...
    // Extract candidate username from /@username path pattern
    ...
    const acct = `${username}@${hostname}`

    // Use WebFinger to verify the account and get the canonical subject
    const webfingerUrl = new URL(`https://${hostname}/.well-known/webfinger`)
    ...
    if (!subjectValue?.startsWith('acct:')) {
      return null
    }

    return `@${subjectValue.slice('acct:'.length)}`
  } catch {
    return null
  }
}
