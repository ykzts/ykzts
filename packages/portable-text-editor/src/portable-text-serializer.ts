      ...
      if ($isImageNode(child)) {
        ...
        for (const rowChild of child.getChildren()) {
          ...
          for (const cellChild of rowChild.getChildren()) {
            if (!$isTableCellNode(cellChild)) {
              continue;
            }

            const isHeader = cellChild.hasHeaderState(
          ...
          if (block.listItem) {
            ...
            while (listStack.length < level) {
              ...
              if (listStack.length > 0) {
                ...
                const parentListInfo = listStack.at(-1);
                if (!parentListInfo) {
                  continue;
                }
                const parentList = parentListInfo.list;
